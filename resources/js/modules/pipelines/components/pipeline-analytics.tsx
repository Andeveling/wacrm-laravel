...
/**
 * Weighted pipeline value: value × per-stage probability. First stage ≈ 10%,
 * interpolates up to 90% before the final stage, final stage (Won) = 100%.
 */
    ...
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-card/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        <Metric
          ...
          tooltip="Negocios abiertos o ganados."
        />
        <Metric
          icon={<DollarSign className="h-4 w-4 text-primary" />}
          label="Valor del pipeline"
          value={formatCurrency(stats.totalValue)}
          tooltip="Suma del valor de todos los negocios activos."
        />
        <Metric
          icon={<Target className="h-4 w-4 text-blue-400" />}
          label="Valor promedio"
          value={formatCurrency(stats.avgValue)}
          tooltip="Valor promedio por negocio activo."
        />
        <Metric
          icon={<TrendingUp className="h-4 w-4 text-purple-400" />}
          label="Valor ponderado"
          value={formatCurrency(stats.weightedValue)}
          tooltip="Valor de negocios abiertos ponderado por probabilidad de cierre según la etapa."
        />
        <Metric
          icon={<Trophy className="h-4 w-4 text-primary" />}
          label="Ganados este mes"
          value={String(stats.wonThisMonth)}
          tooltip="Negocios marcados como ganados este mes."
        />
        <Metric
          icon={<XCircle className="h-4 w-4 text-red-400" />}
          label="Perdidos este mes"
          value={String(stats.lostThisMonth)}
          tooltip="Negocios marcados como perdidos este mes."
        />
      </div>
    </TooltipProvider>
  );
