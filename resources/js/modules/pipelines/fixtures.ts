import type { Deal, Pipeline, PipelineStage } from './contracts';

export function mockDealContacts(count = 20) {
  return Array.from({ length: count }, (_, index) => ({
    id: `pipeline-contact-${index}`,
    name: `Contacto ${index + 1}`,
    phone: `+57 304 000 ${String(index).padStart(4, '0')}`,
  }));
}
...
export function mockDeals(count = 14): Deal[] {
  const contacts = mockDealContacts(count);
